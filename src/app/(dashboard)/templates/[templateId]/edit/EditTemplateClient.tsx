...
export function EditTemplateClient({ template }: EditTemplateClientProps) {
  ...
  const [videoSource, setVideoSource] = useState<"local" | "cloudinary" | "url">(
    template.video_url && !template.video_url.includes("/uploads/") ? "url" : "local"
  );
  ...
  const [templateVideoUrl, setTemplateVideoUrl] = useState(
    template.video_url && !template.video_url.includes("/uploads/") ? template.video_url : ""
  );

  // M&E Audio States
  const [mneSource, setMneSource] = useState<"local" | "cloudinary" | "url">(
    template.mne_audio_url && !template.mne_audio_url.includes("/uploads/") ? "url" : "local"
  );
  const [mneAudioFile, setMneAudioFile] = useState<File | null>(null);
  const [mneAudioUrl, setMneAudioUrl] = useState(
    template.mne_audio_url && !template.mne_audio_url.includes("/uploads/") ? template.mne_audio_url : ""
  );

  const initialAudioSources = (template.audio_sources || []).map((src) => ({
    label: src.name,
    sourceType: (src.url.includes("/uploads/") ? "local" : "url") as "local" | "cloudinary" | "url",
  ...
  const handleUpdateTemplate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(err instanceof Error ? err.message : "Gagal menyimpan perubahan template");
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="max-w-2xl mx-auto space-y-6 pb-12">
      ...
      <Card className="border border-border/50 shadow-lg">
        <CardHeader className="space-y-1">
          <CardTitle className="text-2xl font-bold tracking-tight">Edit Template: {template.name}</CardTitle>
          <CardDescription>
            Perbarui detail informasi media induk (video/audio) untuk template master ini.
          </CardDescription>
        </CardHeader>
