  ...
  return (
    <div className="p-4 space-y-4">
      <h3 className="text-sm font-bold text-foreground/90">
        Glosarium Kata Kunci (Key Terms)
      </h3>
            ...
            {activeScene?.loops
              ...
              .map((term, index) => (
                <tr key={index} className="hover:bg-muted/20">
                  ...
                  {/* UBAH DI SINI: Menggunakan data asli dan menambahkan whitespace-pre-line */}
                  <td className="p-3 text-muted-foreground whitespace-pre-line leading-relaxed">
                    {term.meaning_or_note || "-"}
                  </td>
                </tr>
              ))}
            {(!activeScene ||
              activeScene.loops.flatMap((l) => l.key_terms).length === 0) && (
              <tr>
                <td
                  colSpan={3}
