...
export function DraftTab({
  ...
  return (
    <div className="divide-y divide-zinc-850">
      {activeScene && (
        <div className="grid grid-cols-2 gap-4 px-3.5 py-1.5 bg-zinc-900/30 text-[10px] font-black uppercase tracking-wider text-white select-none text-center items-center">
          <div className="pr-4 border-r border-zinc-800 flex items-center justify-center gap-1.5">
            <span className="text-zinc-500 font-bold uppercase tracking-wider text-[10px]">Referensi:</span>
            <select
              value={activeAudioUrl}
              onChange={(e) => {
                setActiveAudioUrl(e.target.value);
              }}
              className="bg-zinc-950 border border-zinc-850 text-[10px] font-mono rounded px-2 py-0.5 text-white focus:outline-none focus:border-zinc-700 cursor-pointer"
            >
      ...
      {activeScene ? (
        loopsWithDisplay.map(({ loop, text, note }) => {
          ...
          return (
            <div key={loop.id} className="p-3.5 grid grid-cols-2 gap-4 hover:bg-zinc-900/10 transition-colors">
              {/* Left Column: Play, Name, Text */}
              <div className="flex items-start gap-4 pr-4 border-r border-zinc-800">
              ...
              <div className="flex flex-col gap-2 pl-4 justify-center">
                <div className="flex items-center gap-3 w-full">
                  ...
                  <input
                    ...
                    className="flex-1 min-w-[80px] bg-zinc-900 border border-zinc-800 rounded px-2.5 py-1 text-xs text-zinc-200 placeholder-zinc-600 focus:outline-none focus:border-zinc-700 focus:bg-zinc-950 transition-colors"
                  />
